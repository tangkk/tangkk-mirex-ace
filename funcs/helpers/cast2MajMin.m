...
% FIXME: currently only map to majmin or maj7min7
function ch = cast2MajMin(ch,nf)

if ~isempty(strfind(ch,'maj7sus2')) || ~isempty(strfind(ch,'7sus2'))...
        ||~isempty(strfind(ch,'maj7sus4')) || ~isempty(strfind(ch,'7sus4'))
    tokens = strsplit(ch,':');
    ...
    if nf(minP) > nf(majP)
        ch = [num2note(root) ':' 'min7'];
    else
        ch = [num2note(root) ':' 'maj7'];
    end
elseif ~isempty(strfind(ch,'maj9sus4')) || ~isempty(strfind(ch,'9sus4'))
    tokens = strsplit(ch,':');
    root = note2num(tokens{1});
    % min: 3,7, maj: 4,7
    minP = pitchTranspose(root,3);
...
elseif ~isempty(strfind(ch,'sus2')) || ~isempty(strfind(ch,'sus4'))
    tokens = strsplit(ch,':');
    root = note2num(tokens{1});
    % min: 3,7, maj: 4,7
    minP = pitchTranspose(root,3);
