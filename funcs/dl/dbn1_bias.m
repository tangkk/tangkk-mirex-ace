...
function model = dbn1_bias(n_hid, nclass, lr_rbm, lr_classification, wd, mini_batch_size, n_iterations, name_dataset)
    % load data
    ...
    % train RBM
    disp('restricted Boltzmann machine 1 training with bias nodes...');
...
%                      n_iterations);
    ...
    disp('discrimination fine-tuning with bias nodes...');
    model = trainDNN1([size(data_sets.training.targets,1), n_hid+1], @(model_, data) d_loss_by_d_model_bias(model_, data, wd),...
    ...
    for data_details = reshape({'training', data_sets.training, 'validation', data_sets.validation, 'test', data_sets.test}, [2, 3]),
        ...
        hid_input = input_to_hid * [ones(1,m);data.inputs]; % size: <number of hidden units> by <number of data cases>
        hid_output = logistic(hid_input); % size: <number of hidden units> by <number of data cases>
        class_input = hid_to_class * [ones(1,m);hid_output]; % size: <number of classes> by <number of data cases>
