function model = trainDNN2(model_shape, gradient_function, training_data, input_to_hid_1, hid_1_to_hid_2, hid_2_to_class, learning_rate, mini_batch_size, n_iterations)
    ...
    for iteration_number = 1:n_iterations,
        if mod(iteration_number,100) == 0
			...
			if size(hid_1_to_hid_2,1) ~= model_shape(2)
                loss_2_bias(model,training_data);
            else
                loss_2(model,training_data);
            end
        end
