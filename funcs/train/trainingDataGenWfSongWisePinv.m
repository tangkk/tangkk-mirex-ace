...
function trainingDataGenWfSongWisePinv(savename, gtList, noinv, no7)
...
while ischar(tline)
    ...
    tw = ((1/feparam.fs)*(1:bdrys(end))); % time ticks
...
for ii = 1:nsongs
    ...
    % transform X to 12 keys directly
    Xcq_ = cqt(trainingDataOneSongX1, B, fs, fmin, fmax, 'rasterize', 'full', 'gamma', gamma, ...
    ...
    for j = 0:5
        ...
        TXt = icqt(Xcq);
        trainingDataOneSongX11(12-j,:) = TXt;
    end
    ...
    for j = 1:6
        ...
        TXt = icqt(Xcq);
        trainingDataOneSongX11(j,:) = TXt;
    end
    
    % transform y one by one
    idx = 1;
    for i = 1:len
        yi = trainingDataOneSongy(i);
        if i > 1 && yi == trainingDataOneSongy(i-1)
            trainingDataOneSongy11(:,idx) = trainingDataOneSongy11(:,idx-1);
        else
            if yi ~= 0
                ynum = chordnums{yi};
            else
