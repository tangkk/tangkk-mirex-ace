function bnet4 = dbnSetup4(dbnparam, nchords)
% HMM specification:
...
bnet4 = mk_dbn(intra, inter, ns, 'discrete', dnodes, 'observed', cnodes,...
...
bnet4.CPD{1} = tabular_CPD(bnet4, 1, prior);
bnet4.CPD{2} = gaussian_CPD(bnet4, 2, 'mean', mu, 'cov', sigma);
bnet4.CPD{3} = tabular_CPD(bnet4, 3, transmat);
