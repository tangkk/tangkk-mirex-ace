...
function tangkkace_cvt(testingParams,putget)
...
for cvi = 1:5
    ...
    for dsi = 1:length(dataset)
        ...
        % third loop, loop over training folds
        for cvt = 1:4
            thisfold = str2double(trainfolds(cvt));
            testlist = ['data/cvlist/', thisset, '-', num2str(thisfold), '.txt'];
            if strcmp(featurelevel,'ch')
                loadtmp = ['bub/', thisset, 'BUB', '-', num2str(thisfold)];
            elseif strcmp(featurelevel,'ns')
                loadtmpch = ['bub/', thisset, 'BUB', '-', num2str(thisfold)];
                loadtmpns = ['bub/', thisset, 'BUBns', '-', num2str(thisfold)];
            else
                loadtmp = [];
                loadtmpch = [];
                loadtmpns = [];
    ...
    if strcmp(putget,'put')
        ...
        evaluateCP_cvt(evalsuffix,evallistins);
    end
end
