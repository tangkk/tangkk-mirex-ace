...
# it's weird that 'repeat' is performing much better than 'reshape'
#
# fC = 'reshape'
# dim_proj = 12
fC = 'repeat'
...
def build_model(tparams, options, fC):
    ...
    if fC == 'reshape':
        ...
        emb = x.reshape([n_timesteps, n_samples, options['dim_proj']])
    elif fC == 'repeat':
    ...
    try:
        for eidx in xrange(max_epochs):
            ...
            for _, train_index in kf:
                ...
                x, mask, y = prepare_data(x, y, dim_proj=dim_proj, fC=fC)
                n_samples += x.shape[1]
