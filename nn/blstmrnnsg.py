...
def param_init_lstm(options, params, prefix='lstm'):
    ...
    W_0 = numpy.concatenate([ortho_weight(options['xdim'],options['dim_proj_0']),
                           ortho_weight(options['xdim'],options['dim_proj_0']),
                           ortho_weight(options['xdim'],options['dim_proj_0']),
                           ortho_weight(options['xdim'],options['dim_proj_0'])], axis=1)
    params[_p(prefix, 'W_0')] = W_0 # "lstm_W"
    # U are recurrent weights
    U_0 = numpy.concatenate([ortho_weight(options['dim_proj_0'],options['dim_proj_0']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj_0']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj_0']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj_0'])], axis=1)
    params[_p(prefix, 'U_0')] = U_0 # "lstm_U"
    # b_0 are bias
    b_0 = numpy.zeros((4 * options['dim_proj_0'],))
    params[_p(prefix, 'b_0')] = b_0.astype(config.floatX) # "lstm_b"

    # these are for the backward pass
    Wb_0 = numpy.concatenate([ortho_weight(options['xdim'],options['dim_proj_0']),
                           ortho_weight(options['xdim'],options['dim_proj_0']),
                           ortho_weight(options['xdim'],options['dim_proj_0']),
                           ortho_weight(options['xdim'],options['dim_proj_0'])], axis=1)
    params[_p(prefix, 'Wb_0')] = Wb_0 # "lstm_Wb"
    # U are recurrent weights
    Ub_0 = numpy.concatenate([ortho_weight(options['dim_proj_0'],options['dim_proj_0']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj_0']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj_0']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj_0'])], axis=1)
    params[_p(prefix, 'Ub_0')] = Ub_0 # "lstm_Ub"
    # b_0 are bias
    bb_0 = numpy.zeros((4 * options['dim_proj_0'],))
    params[_p(prefix, 'bb_0')] = bb_0.astype(config.floatX) # "lstm_bb"
    
    
    
    # these are for the forward pass
    W = numpy.concatenate([ortho_weight(options['dim_proj_0'],options['dim_proj']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj'])], axis=1)
    ...
    # these are for the backward pass
    Wb = numpy.concatenate([ortho_weight(options['dim_proj_0'],options['dim_proj']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj']),
                           ortho_weight(options['dim_proj_0'],options['dim_proj'])], axis=1)
...
def lstm_layer(tparams, x, y, use_noise, options, prefix='lstm'):
    ...
    def _step(x_, xb_, h_, z_, c_, d_, hb_, zb_, cb_, db_):
        
        # forward pass
        preact_0 = T.dot(h_, tparams[_p(prefix, 'U_0')])
        preact_0 += T.dot(x_, tparams[_p(prefix, 'W_0')])
        preact_0 += tparams[_p(prefix, 'b_0')]

        i_0 = T.nnet.sigmoid(_slice(preact_0, 0, options['dim_proj_0']))
        f_0 = T.nnet.sigmoid(_slice(preact_0, 1, options['dim_proj_0']))
        o_0 = T.nnet.sigmoid(_slice(preact_0, 2, options['dim_proj_0']))
        c = T.tanh(_slice(preact_0, 3, options['dim_proj_0']))

        c = f_0 * c_ + i_0 * c
        h = o_0 * T.tanh(c)
        
        preact = T.dot(z_, tparams[_p(prefix, 'U')])
        preact += T.dot(h, tparams[_p(prefix, 'W')])
        ...
        d = T.tanh(_slice(preact, 3, options['dim_proj']))
        ...
        preactb_0 = T.dot(hb_, tparams[_p(prefix, 'Ub_0')])
        preactb_0 += T.dot(xb_, tparams[_p(prefix, 'Wb_0')])
        preactb_0 += tparams[_p(prefix, 'bb_0')]

        ib_0 = T.nnet.sigmoid(_slice(preactb_0, 0, options['dim_proj_0']))
        fb_0 = T.nnet.sigmoid(_slice(preactb_0, 1, options['dim_proj_0']))
        ob_0 = T.nnet.sigmoid(_slice(preactb_0, 2, options['dim_proj_0']))
        cb = T.tanh(_slice(preactb_0, 3, options['dim_proj_0']))

        cb = fb_0 * cb_ + ib_0 * cb
        hb = ob_0 * T.tanh(cb)
        
        preactb = T.dot(zb_, tparams[_p(prefix, 'Ub')])
        preactb += T.dot(hb, tparams[_p(prefix, 'Wb')])
        ...
        db = T.tanh(_slice(preactb, 3, options['dim_proj']))

        db = fb * db_ + ib * db
        zb = ob * T.tanh(db)
        
        # take the reverse of hb and concatenate with h before feeding into logistic regression
        zzb = T.concatenate([z,zb[::-1]])
        # a single frame prediction given h - the posterior probablity
        one_pred = T.nnet.softmax(T.dot(zzb, tparams['U']) + tparams['b'])
        
        return h, z, c, d, hb, zb, cb, db, one_pred
    
    dim_proj_0 = options['dim_proj_0']
    dim_proj = options['dim_proj']
    ydim = options['ydim']
    # the scan function takse one dim_proj vector of x and one target of y at a time
    # the output is:
    # rval[0] -- n_timesteps of h -- n_timesteps * dim_proj_0
    # rval[1] -- n_timesteps of z -- n_timesteps * dim_proj
    # rval[2] -- n_timesteps of c -- n_timesteps * dim_proj_0
    # rval[3] -- n_timesteps of d -- n_timesteps * dim_proj
    # rval[4] -- n_timesteps of hb -- n_timesteps * dim_proj_0
    # rval[5] -- n_timesteps of zb -- n_timesteps * dim_proj
    # rval[6] -- n_timesteps of cb -- n_timesteps * dim_proj_0
    # rval[7] -- n_timesteps of db -- n_timesteps * dim_proj
    # rval[8] -- n_timesteps of one_pred -- n_timesteps * ydim
...
def predprobs(model, X):
    ...
    model_options['xdim'] = model['lstm_W_0'].shape[0]
    model_options['dim_proj_0'] = model['lstm_W_0'].shape[1] / 4
...
def train_lstm(
    ...
    model_options['dim_proj_0'] = dim_proj_0
...
if __name__ == '__main__':
    ...
    dim_proj_0 = int(sys.argv[5])
    dim_proj = int(sys.argv[6])
