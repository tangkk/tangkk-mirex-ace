...
def param_init_lstm(options, params, prefix='lstm'):
    ...
    :see: init_params
    """
    # W are the input weights (maps xdim to dim_proj_0)
    ...
    # two sets of lstm weights -- two hidden lstm layers
...
def lstm_layer(tparams, x, y, use_noise, options, prefix='lstm'):
    ...
    def _step(x_, h_, z_, c_, d_):
        ...
        preact_0 += T.dot(x_i, tparams[_p(prefix, 'W_0')])
        preact_0 += tparams[_p(prefix, 'b_0')]
        # the first hidden lstm layer is with dim_proj_0 size lstm units
        i_0 = T.nnet.sigmoid(_slice(preact_0, 0, options['dim_proj_0'])) # input gate
        f_0 = T.nnet.sigmoid(_slice(preact_0, 1, options['dim_proj_0'])) # forget gate
        o_0 = T.nnet.sigmoid(_slice(preact_0, 2, options['dim_proj_0'])) # output gate
        c = T.tanh(_slice(preact_0, 3, options['dim_proj_0'])) # previous cell state
        ...
        # the second hidden lstm layer is with dim_proj size lstm units
        i = T.nnet.sigmoid(_slice(preact, 0, options['dim_proj'])) # input gate
        f = T.nnet.sigmoid(_slice(preact, 1, options['dim_proj'])) # forget gate
        o = T.nnet.sigmoid(_slice(preact, 2, options['dim_proj'])) # output gate
        d = T.tanh(_slice(preact, 3, options['dim_proj'])) # previous cell state
        ...
        one_pred = T.nnet.softmax(T.dot(z, tparams['U']) + tparams['b'])
        
        return h, z, c, d, one_pred
    ...
    # rval[3] -- n_timesteps of c -- n_timesteps * dim_proj_0
    # rval[4] -- n_timesteps of d -- n_timesteps * dim_proj
    # rval[5] -- n_timesteps of one_pred -- n_timesteps * ydim
...
def predprobs(model, X):
    # FIXME: fix the whole function when there are more than one lstm hidden layers
